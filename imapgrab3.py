...
def imapgrab():
    ...
    ig_cmd.add_option("-l", "--list", action="store_true",
                      help="List the mailboxes available for download")
    ig_cmd.add_option("-d", "--download", action="store_true",
                      help="Download mailboxes to separate files/folders")
    ig_cmd.add_option("-B", "--mbox", action="store_true",
                      help="Download into Mbox format (optional, default)")
    ig_cmd.add_option("-M", "--maildir", action="store_true",
                      help="Download into Maildir format (optional)")
    ig_cmd.add_option("-a", "--all", action="store_true",
                      help="Force download all mail in a mailbox (optional)")
    ig_cmd.add_option("-S", "--ssl", action="store_true",
                      help="Use SSL connection (optional)")
    ig_cmd.add_option("-s", "--server", dest="server",
                      help="IP or domain of server (required)")
    ig_cmd.add_option("-P", "--port", dest="port",
                      help="Port of server (optional)")
    ig_cmd.add_option("-u", "--username", dest="username",
                      help="Username for account (required)")
    ig_cmd.add_option("-p", "--password", dest="password",
                      help="Password for account (required)")
    ...
    ig_cmd.add_option("-q", "--quiet", action="store_true",
                      help="Don't display any output")
    ig_cmd.add_option("-v", "--verbose", action="store_true",
                      help="Verbose output")
    ig_cmd.add_option("--debug", action="store_true",
                      help="Print debug output")
    ig_cmd.add_option("--version", action="store_true", help="Print version")
    ig_cmd.add_option("--about", action="store_true",
                      help="Display detailed info")
...
def IG_backup_mail(ig_options):
    ...
    if "_ALL_" in ig_download_list:
        ...
        ig_download_list = filter(
            lambda x: x not in ig_exceptions_list, ig_list)
    ...
    for i in ig_download_list:
        ...
        if ig_options.maildir is True:
            ...
            rc_file.write("path = " + fpath + os.sep +
                          filename + os.sep + "\n\n")
        else:
        ...
        if (ig_options.debug is True):
            print("DEBUG_024: Contents of \"" +
                  fpath + os.sep + filename + ".rc\":")
            print("--------------------")
        ...
        if ig_options.maildir is True:
            ...
            maildir_directories = ["", "new" + os.sep,
                                   "cur" + os.sep, "tmp" + os.sep]
            ...
            if os.path.isfile(fpath + os.sep + filename + ".mbox") is not True:
                ...
                if (ig_options.debug is True):
                    print("DEBUG_026: \"" + fpath + os.sep +
                          filename + ".mbox\" exists")
    ...
    for mailbox in ig_download_list:
        ...
        if ig_options.localuser is not None:
            ...
            os.system("chown -R " + ig_options.localuser +
                      " " + ig_options.folder)
        ...
        if (ig_options.quiet is not True):
            print("Downloading mailbox \"" + mailbox_str + "\"")
        if (ig_options.debug is True):
            print(
